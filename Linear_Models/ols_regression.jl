...
# ╔═╡ 1c2bb024-e4c6-11eb-1df3-4de48dfaca96
using CSV, DataFrames,LinearAlgebra, Statistics

# ╔═╡ 2822cee5-2cc8-46be-86fd-32ec54cfb564
Normalize(X)=(X .- findmin(X,dims=1)[1]) ./ (findmax(X,dims=1)[1] - findmin(X, dims=1)[1])

# ╔═╡ 4ec72cae-3ff0-44eb-a68e-31777d0ec462
ols(X,y)= pinv(X' * X) * X' * y

# ╔═╡ 4cca2466-206a-4925-ac7d-dbd93c98e0ce
ridge_ols(X,y,λ) = pinv(X' * X + λ * I) * X' * y

# ╔═╡ 975df829-c701-4a59-8fa8-b639a3e0ee42
lm(X,B)= X * B

# ╔═╡ 83b3bdb9-1a8e-4d46-837b-31939fb32a8d
RMSE(y,ŷ) = sqrt.(sum(ŷ-y).^2/ size(y)[1])

# ╔═╡ 7131a049-81ad-46cb-9a49-21c7ef3029fc
wine = DataFrame(CSV.File("/home/john/Documents/julia_data/winequality-red.csv"));

# ╔═╡ 4a72301a-0975-4ac7-b014-e0a04d9a9cd8
begin
...
# ╔═╡ 1014a566-3c52-410b-9efc-e99ee22a6d3b
RMSE(y,lm(X,ridge_ols(X,y,1)))

# ╔═╡ 486b0fe4-26f2-4ad8-8f56-e2ba29c757ce
RMSE(y,lm(X,ols(X,y)))
